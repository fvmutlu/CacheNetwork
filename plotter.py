...
all_graphs = ['cycle', 'lollipop', 'grid_2d', 'balanced_tree', 'hypercube', 'expander', 'erdos_renyi', 'regular',
              'watts_strogatz', 'small_world', 'barabasi_albert', 'cicular_ladder', 'star', 'powerlaw_tree', 'geant', 'abilene', 'dtelekom']
threshold = 1000.0
...
if __name__ == '__main__':
    ...
    for filename in myargs.filenames:
        ...
        try:
            with open(filename, 'rb') as f:
                args, construct_stats, optimal_stats, demand_stats, node_stats, network_stats = pickle.load(
                    f)
    ...
    ecg_list = {}
    for cache in caches:
        ecg_list[cache] = [0.0 if cache not in ecg_avg[graph]
                           else ecg_avg[graph][cache] for graph in graphs]
    ...
    for cache in caches:
        print(newind)
        if myargs.hatch:
            rects += ax.bar(newind, ecg_list[cache], width, color=cmaps.plasma(
                0.5 + 0.5 * i / len(caches)), hatch=hatchmap[cache], linewidth=2, label=cache)
        else:
            rects += ax.bar(newind, ecg_list[cache], width, color=cmaps.plasma(
                0.5 + 0.5 * i / len(caches)), linewidth=1, label=cache)
        newind += width
        i += 1
    ...
    if myargs.legend:
        ...
        # plt.tight_layout()
        fig.savefig('barplot%s%s.pdf' % ('_'.join(graphs), hatchstring),
                    bbox_extra_artists=(lgd,), bbox_inches='tight')
    else:
        fig.savefig('barplot%s%s.pdf' %
                    ('_'.join(graphs), hatchstring), bbox_inches='tight')
    plt.close(fig)
